...
public class CypherTool {
    ...
    private static String readNextToken(Scanner scanner) {
        if (!scanner.hasNext()) {
...
=======
            if (input.getOperation() == 1) {
                res = switch (input.getChoice()) {
                    case 1 -> encryptRot13(input.getMessage());
                    case 2 -> encryptAtbash(input.getMessage());
                    case 3 -> encryptCaesar(input.getMessage());

                    default -> "Invalid Choice of encyption";
                };
// decryption of the message
            } else if (input.getOperation() == 2) {
                res = switch (input.getChoice()) {
                    case 1 -> decryptRot13(input.getMessage());
                    case 2 -> decryptAtbash(input.getMessage());
                    case 3 -> decryptCaesar(input.getMessage());

>>>>>>> 969c1db972d130c854c2838d5fbd82d69bf382cc
    ...
    public static InputData getInput() {
        ...
        while (true) {
            System.out.print("""
                    ...
                    2. Decrypt
                    :>\s""");
            String exitInput = readNextToken(scanner);
...
=======
        System.out.print("Enter your message: ");
        String message = readNextLine(scanner);
